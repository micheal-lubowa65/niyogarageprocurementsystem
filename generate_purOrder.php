<?php
require_once('tcpdf/tcpdf.php'); // Including the TCPDF library
...
if (mysqli_num_rows($result) > 0) {
    ...
    // Adding title with a larger, bold font
    $pdf->SetFont('helvetica', 'B', 16);
    $pdf->Cell(0, 10, 'Niyo Garage Purchase Order', 0, 1, 'C');
    $pdf->Ln(5);

    // Seting font back to regular for content
    $pdf->SetFont('helvetica', '', 12);

    // Setting a background color for the headers
    $pdf->SetFillColor(230, 230, 230);  // Light gray
    $pdf->Cell(0, 10, 'Requester Details', 0, 1, 'L', true);  // Section header
    $pdf->Ln(2);

    // Displaying the data from the row with some styling
    $pdf->SetFont('helvetica', '', 12);
